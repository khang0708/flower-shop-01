...
export const generateMessengerProductInquiry = (pageId, product) => {
  ...
  const message = `Chào Ngọc Flower, tôi muốn tư vấn mẫu hoa "${product.name}" (giá: ${priceStr}đ). Nhờ tiệm gửi thêm ảnh hoa thực tế tại tiệm giúp tôi nhé!`;
...
export const generateMessengerAIInquiry = (pageId, analysisResult) => {
  ...
  const message = `Chào nghệ nhân Ngọc Flower, tôi vừa dùng AI Florist Vision thẩm định mẫu hoa phong cách "${style}" (${detected}). Nhờ tiệm báo giá và tư vấn cắm mẫu này giúp tôi nhé!`;
  return openFacebookMessenger(pageId, message);
};
