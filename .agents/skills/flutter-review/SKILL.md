...
1. **Build passes** â€” run `/flutter-build` first; a review on broken code is incomplete
2. **Tests pass** â€” run `/flutter-test` to confirm no regressions
3. **No merge conflicts** â€” resolve all conflicts so the diff reflects only intentional changes
4. **`flutter analyze` is clean** â€” fix analyzer warnings before review
...
Issue: `Text('Login')` â€” user-visible string not using localization system.
...
- `/flutter-build` â€” Fix build errors first
- `/flutter-test` â€” Run tests before reviewing
- `/code-review` â€” General code review (language-agnostic)
