...
Surface for `workflows/orch-review.workflow.js` â€” the native Workflow port of
...
| Input | Mode |
|---|---|
| Blank | **Local Mode** â€” review uncommitted changes |
| Number (e.g. `42`) or PR URL | **PR Mode** â€” review a GitHub PR |
...
First derive a **safe numeric PR id** from `$ARGUMENTS` â€” never pass the raw
...
when the change is mixed or non-code â€” the workflow simply skips the
...
Workflow({
  ...
  args: {
    ...
    changedFiles: ["src/auth.ts"]                // optional â€” feeds the security trigger
  }
})
```
...
- List every `blocking` finding with file, severity, and evidence â€” these must
...
run. If the Workflow tool itself errors, report the failure â€” do not fall back to
...
- **Binary or generated files**: drop them from `changedFiles` before invoking â€”
