...
Analyzing project...
  ...
  error â€¢ lib/features/user/data/user_repository_impl.dart:34:16 â€¢
    A value of type 'String?' can't be assigned to a variable of type 'String'. â€¢
    invalid_assignment

  error â€¢ lib/features/cart/presentation/cart_page.dart:58:5 â€¢
    The method 'add' isn't defined for the type 'List<Item>'.
    Try correcting the name to the name of an existing method, or defining a method named 'add'. â€¢
    undefined_method
...
// .add(event) is the BLoC event API â€” don't mix them.
```
...
1. **Analysis errors first** â€” code must be error-free
2. **Warning triage second** â€” fix warnings that could cause runtime bugs
3. **pub conflicts third** â€” fix dependency resolution
4. **One fix at a time** â€” verify each change
5. **Minimal changes** â€” don't refactor, just fix
...
- `/flutter-test` â€” Run tests after build succeeds
- `/flutter-review` â€” Review code quality
