---
name: react-build
description: Fix React build failures (Vite, webpack, Next.js, CRA, Parcel, esbuild, Bun) incrementally â€” JSX/TSX compile errors, hydration mismatches, server/client component boundary failures, missing types. Invokes the react-build-resolver agent for minimal, surgical fixes.
---
...
| `Invalid hook call` | Multiple React copies â€” dedupe via `resolutions`/`overrides` |
| `Element type is invalid` | Default vs named import mismatch |

## Fix Strategy

1. **Compile errors first** â€” code must build
2. **Hydration errors second** â€” affects production correctness
3. **Bundler config third** â€” restore plugin/loader correctness
4. **One fix at a time** â€” verify each change
5. **Minimal changes** â€” never `// @ts-ignore` without explanation
6. **Re-run after each fix** â€” surface new errors immediately
...
- `/react-test` â€” run tests after the build is green
- `/react-review` â€” review code quality after the build succeeds
- `/build-fix` â€” generic build fixer (non-React)
- `verification-loop` skill â€” full verification loop
