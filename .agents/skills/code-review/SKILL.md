---
name: code-review
description: Code review â€” local uncommitted changes or GitHub PR (pass PR number/URL for PR mode)
...
â†’ Jump to **PR Review Mode** below.

Otherwise:
â†’ Use **Local Review Mode**.
...
Comprehensive GitHub PR review â€” fetches diff, reads full files, runs validation, posts review.
...
1. **Project rules** â€” Read `CLAUDE.md`, `.claude/docs/`, and any contributing guidelines
2. **Planning artifacts** â€” Check `.claude/prds/`, `.claude/plans/`, `.claude/reviews/`, and legacy `.claude/PRPs/{prds,plans,reports,reviews}/` for context related to this PR
3. **PR intent** â€” Parse PR description for goals, linked issues, test plans
4. **Changed files** â€” List all modified files and categorize by type (source, test, config, docs)

### Phase 3 â€” REVIEW

Read each changed file **in full** (not just the diff hunks â€” you need surrounding context).
...
Special cases:
- Draft PR â†’ Always use **COMMENT** (not approve/block)
- Only docs/config changes â†’ Lighter review, focus on correctness
- Explicit `--approve` or `--request-changes` flag â†’ Override decision (but still report all findings)
