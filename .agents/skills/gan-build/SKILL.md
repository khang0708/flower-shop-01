...
1. `brief` â€” the user's one-line description of what to build
2. `--max-iterations N` â€” (optional, default 15) maximum generator-evaluator cycles
3. `--pass-threshold N` â€” (optional, default 7.0) weighted score to pass
4. `--skip-planner` â€” (optional) skip planner, assume spec.md already exists
5. `--eval-mode MODE` â€” (optional, default "playwright") one of: playwright, screenshot, code-only
...
while iteration <= max_iterations:
    ...
    if iteration >= 3 and score has not improved in last 2 iterations:
        Log "PLATEAU detected â€” stopping early"
        Break
...
3. Show score progression: `iteration 1: 4.2 â†’ iteration 2: 5.8 â†’ ... â†’ iteration N: 7.5`
