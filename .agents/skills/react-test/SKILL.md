...
2. **Write Behavior Tests First**: RTL queries (role-first), `userEvent`, MSW for network â€” RED
3. **Run Tests**: Verify they fail for the right reason
4. **Implement Minimal Code**: Just enough to pass â€” GREEN
...
Ã— src/components/SearchInput.test.tsx (4 tests) âœ˜ Error: not implemented
```
...
âœ“ src/components/SearchInput.test.tsx (4 tests) 47ms
```
...
- `container.querySelector(...)` â€” bypasses accessibility queries
...
- Ignoring `act()` warnings â€” they signal real bugs
- Snapshot tests of rendered components (brittle, rubber-stamped) â€” use Playwright/Cypress visual diff instead
...
- `/react-build` â€” fix build errors before running tests
- `/react-review` â€” review after implementation
