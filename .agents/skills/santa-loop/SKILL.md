---
name: santa-loop
description: Adversarial dual-review convergence loop â€” two independent model reviewers must both approve before code ships.
---

# Santa Loop

Adversarial dual-review convergence loop using the santa-method skill. Two independent reviewers â€” different models, no shared context â€” must both return NICE before code ships.

## Purpose

Run two independent reviewers (Claude Opus + an external model) against the current task output. Both must return NICE before the code is pushed. If either returns NAUGHTY, fix all flagged issues, commit, and re-run fresh reviewers â€” up to 3 rounds.
...
The verdict gate (Step 4) maps these to NICE/NAUGHTY: both PASS â†’ NICE, either FAIL â†’ NAUGHTY.
...
Launch a second Claude Agent (subagent_type: `code-reviewer`, model: `opus`). Log a warning that both reviewers share the same model family â€” true model diversity was not achieved but context isolation is still enforced.

In all cases, the reviewer must return the same structured JSON verdict as Reviewer A.

### Step 4: Verdict Gate

- **Both PASS** â†’ **NICE** â€” proceed to Step 6 (push)
- **Either FAIL** â†’ **NAUGHTY** â€” merge all critical issues from both reviewers, deduplicate, proceed to Step 5

### Step 5: Fix Cycle (NAUGHTY path)

1. Display all critical issues from both reviewers
2. Fix every flagged issue â€” change only what was flagged, no drive-by refactors
...
- Reviewer A (Claude Opus) always runs â€” guarantees at least one strong reviewer regardless of tooling.
- Model diversity is the goal for Reviewer B. GPT-5.4 or Gemini 2.5 Pro gives true independence â€” different training data, different biases, different blind spots. The Claude-only fallback still provides value via context isolation but loses model diversity.
...
- Push only happens after NICE â€” never mid-loop.
