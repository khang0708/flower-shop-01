...
This command invokes the **vue-reviewer** agent for Vue-specific code review. For pull requests touching `.vue` files or Vue-containing `.ts`/`.js` files, both `vue-reviewer` and `typescript-reviewer` should run â€” each owns a distinct lane.
...
| `typescript-reviewer` | Generic TS/JS â€” `any` abuse, async correctness, Node security |
